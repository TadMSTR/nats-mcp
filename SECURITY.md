...
- **GitHub private disclosure:** Use the [Security tab](https://github.com/TadMSTR/nats-mcp/security/advisories/new) to submit a private advisory.
- **Email:** Send a description to `security.i9v75@8alias.com` with the subject line `[nats-mcp] Security Report`.
...
- SSRF via the NATS server URL configuration
- Message injection allowing unauthorized publish to NATS subjects
- Subject namespace escapes granting access beyond the intended scope
- Credential exposure through NATS connection credentials in error messages or log output
- Dependency vulnerabilities with a plausible exploitation path in nats-mcp's usage
